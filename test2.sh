#!/bin/bash
# 写一个给定开始和结束日期，就能生成所有的日期序列的shell函数
#
if [[ $# -le 2 || $# -gt 3 ]]; then 
...
function genAlldate{ 
    ...
    while (( "${START_DAY_TMP}" <= "${END_DAY_TMP}" )); 
    do cur_day=$(date -d @${START_DAY_TMP} +"%Y${SPLITER_TMP}%m${SPLITER_TMP}%d") 
...
genAlldate "${START_DAY}" "${END_DAY}" "${SPLITER}"  # [Method 1] Traverse the array.
